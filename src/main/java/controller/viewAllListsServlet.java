...
public class viewAllListsServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List<ListDetails> l = ldh.getLists();
		request.setAttribute("allLists", l);
		
		if(l.isEmpty()) {
			request.setAttribute("allLists", " ");
		}
		
		getServletContext().getRequestDispatcher("/breeder-list-by-user.jsp").forward(request, response);
	}
